...
function TurnForm({initial,plants,trucks,users,concreteTypes,allTurns,currentUser,onSave,onClose}){
  ...
  return(<div style={S.modalOverlay}><div style={{...S.modal,maxWidth:600}}><div style={S.modalHeader}><h2 style={S.modalTitle}>{initial?"Editar Turno":"Nuevo Turno"}</h2><button onClick={onClose} style={S.modalClose}>✕</button></div><div style={{...S.formGrid,gap:14}}>
    ...
    <div style={{...S.formGroup,gridColumn:"1/-1"}}>
      ...
      <input type="number" min="0.01" max="100" step="0.01" value={form.m3} onChange={e=>set("m3",e.target.value===''?0:parseFloat(e.target.value))} style={{...S.input,maxWidth:120}}/>
    </div>
  ...
  </div><div style={S.modalFooter}><button onClick={onClose} style={S.btnSecondary}>Cancelar</button><button onClick={()=>{if(canSave)onSave(form);}} style={{...S.btnPrimary,opacity:canSave?1:.45,cursor:canSave?"pointer":"not-allowed"}}>{initial?"Guardar":"Crear turno"}</button></div></div></div>);}
