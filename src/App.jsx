...
async function runGeofencing(turns, vehicles, plantsGeo, onUpdateStatus) {
  ...
  for(const turn of activeTurns) {
    const truckNames = turn.trucks||[];
    // Buscar vehículos de Infotrak que correspondan a este turno
    const turnVehicles = vehicles.filter(v=>
      truckNames.some(name=>v.nombre_hormiturn&&v.nombre_hormiturn.toLowerCase()===name.toLowerCase())
    );
    if(!turnVehicles.length) continue;

    // Usar el primer vehículo del turno como referencia
    const veh = turnVehicles[0];
    const vLat = veh.latitud;
    const vLng = veh.longitud;
    const isMoving = veh.velocidad > 2;

    // Encontrar la planta del turno
    ...
    const enPlanta = distPlanta < (plantGeo?.radioPlanta||200);

    // Distancia a la obra (si tiene coordenadas geocodificadas)
    let enObra = false;
    if(turn.destLat&&turn.destLng) {
      const distObra = distancia(vLat,vLng,turn.destLat,turn.destLng);
      enObra = distObra < RADIO_OBRA;
    }

    const now = Date.now();
    const started = now >= turn.scheduledAt;

    let nuevoEstado = null;

    if(turn.status==="pendiente" && enPlanta && started) {
      nuevoEstado = "en_planta";
    } else if(turn.status==="en_planta" && !enPlanta && isMoving) {
      nuevoEstado = "en_ruta";
    } else if(turn.status==="en_ruta" && enObra && !isMoving) {
      nuevoEstado = "descargando";
    } else if(turn.status==="descargando" && !enObra && enPlanta) {
      nuevoEstado = "completado";
    } else if(turn.status==="descargando" && !enObra && isMoving && !enPlanta) {
      // Volviendo a planta
    }

    if(nuevoEstado) {
      await onUpdateStatus(turn.id, nuevoEstado);
    }
  }
}
...
function MapView({turns, infotrakConfig, plantsGeo, onUpdateStatus}) {
  ...
  function initMap() {
    ...
    // Dibujar zonas de plantas
    if(plantsGeo) {
      plantsGeo.forEach(p=>{
        L.circle([p.lat,p.lng],{radius:p.radioPlanta,color:"#4F8EF7",fillColor:"#4F8EF7",fillOpacity:0.1,weight:2}).addTo(map);
  ...
  function getVehicleName(id) {
    const v=infotrakConfig?.vehiculos?.find(v=>v.id===id);
    return v?v.nombre:String(id);
  }

  function getActiveTurn(vehicleName) {
    return turns.find(t=>(t.trucks||[]).some(tr=>tr.toLowerCase()===vehicleName.toLowerCase())&&!["completado","cancelado"].includes(t.status));
  }
    ...
    try {
      ...
      if(plantsGeo&&filtered.length) {
        ...
        await runGeofencing(turns,filtered,plantsGeo,async(id,status)=>{
          ...
          if(t){
            logs.push(`${fmtTime(Date.now())} — ${t.client}: ${STATUS_CONFIG[t.status]?.label} → ${STATUS_CONFIG[status]?.label}`);
  ...
  function updateMarkers(vehicleList) {
    ...
    vehicleList.forEach(v=>{
      const name=v.nombre_hormiturn;
      const isMoving=v.velocidad>2;
      const activeTurn=getActiveTurn(name);
  ...
  useEffect(()=>{
    if(!infotrakConfig) return;
    const interval=setInterval(()=>{if(window.L&&mapInstance.current)fetchVehicles();},60000);
    return()=>clearInterval(interval);
  },[infotrakConfig,turns]);
    ...
    <div style={{display:"flex",flexDirection:"column",gap:16,height:"100%"}}>
      ...
      {geoLog.length>0&&(
        <div style={{...S.panel,padding:"14px 16px"}}>
          <div style={{...S.panelTitle,marginBottom:8}}>🤖 Cambios automáticos recientes</div>
...
export default function App() {
  ...
  async function handleSaveTurn(data) {
    // Geocodificar destino al crear/editar
    let destLat=data.destLat||null, destLng=data.destLng||null;
    if(data.destination&&(!destLat||!destLng)) {
      const geo=await geocodificar(data.destination);
  ...
  async function renameUser(o,n){const next=users.map(x=>x===o?n:x);const nextPw={...passwords};if(nextPw[o]!==undefined){nextPw[n]=nextPw[o];delete nextPw[o];}setUsers(next);setPasswords(nextPw);await saveConfig("users",next,{passwords:nextPw});for(const t of turns.filter(t=>t.operator===o))await supabase.from("turns").update({operator:n}).eq("id",t.id);if(currentUser===o)setCurrentUser(n);}
  ...
  async function removeUser(v){const next=users.filter(x=>x!==v);const nextPw={...passwords};delete nextPw[v];setUsers(next);setPasswords(nextPw);await saveConfig("users",next,{passwords:nextPw});}
      ...
      <main style={S.main}>
        <header style={S.header}>
          ...
          <div style={{display:"flex",gap:10}}>
            {view==="dashboard"&&<button onClick={()=>printDayReport(turns,dashDay)} style={S.btnSecondary}>🖨 Imprimir día</button>}
            {view==="calendar"&&<button onClick={()=>printDayReport(turns,calendarDay)} style={S.btnSecondary}>🖨 Imprimir día</button>}
...
function SettingsModal({plants,trucks,users,concreteTypes,passwords,onRenamePlant,onRenameTruck,onRenameUser,onRenameConcreteType,onAddPlant,onAddTruck,onAddUser,onAddConcreteType,onRemovePlant,onRemoveTruck,onRemoveUser,onRemoveConcreteType,onSetPassword,onClose}){const [tab,setTab]=useState("plants");return(<div style={S.modalOverlay}><div style={{...S.modal,maxWidth:540}}><div style={S.modalHeader}><h2 style={S.modalTitle}>⚙ Configuración</h2><button onClick={onClose} style={S.modalClose}>✕</button></div><div style={{display:"flex",gap:4,padding:"14px 24px 0",borderBottom:`1px solid ${C.border}`,flexWrap:"wrap"}}>{[["plants","🏭 Plantas"],["trucks","🚛 Camiones"],["users","👤 Operadores"],["concrete","🪨 Hormigón"],["passwords","🔒 Contraseñas"]].map(([k,l])=>(<button key={k} onClick={()=>setTab(k)} style={{...S.tabBtn,...(tab===k?S.tabBtnActive:{})}}>{l}</button>))}</div><div style={{padding:24,maxHeight:"60vh",overflowY:"auto"}}>{tab==="plants"&&<EditableList items={plants} onRename={onRenamePlant} onAdd={onAddPlant} onRemove={onRemovePlant} label="Plantas" placeholder="Nueva planta…"/>}{tab==="trucks"&&<EditableList items={trucks} onRename={onRenameTruck} onAdd={onAddTruck} onRemove={onRemoveTruck} label="Camiones" placeholder="Nuevo camión…"/>}{tab==="users"&&<EditableList items={users} onRename={onRenameUser} onAdd={onAddUser} onRemove={onRemoveUser} label="Operadores" placeholder="Nuevo operador…"/>}{tab==="concrete"&&<EditableList items={concreteTypes} onRename={onRenameConcreteType} onAdd={onAddConcreteType} onRemove={onRemoveConcreteType} label="Tipos de Hormigón" placeholder="Ej: H-25…"/>}{tab==="passwords"&&<PasswordList users={users} passwords={passwords} onSetPassword={onSetPassword}/>}<p style={{fontSize:12,color:C.muted,marginTop:4}}>Los cambios se sincronizan para todos los usuarios.</p></div><div style={S.modalFooter}><button onClick={onClose} style={S.btnPrimary}>Listo</button></div></div></div>);}
...
function TurnCard({turn:t,onAdvance,onEdit,onCancel,onDelete,compact}){const cfg=STATUS_CONFIG[t.status];const canAdv=STATUS_FLOW.includes(t.status)&&t.status!=="completado";const truckList=(t.trucks||[]).join(", ");const timeRange=t.endAt?`${fmtTime(t.scheduledAt)} → ${fmtTime(t.endAt)}`:`${fmtTime(t.scheduledAt)}`;return(<div style={S.turnCard}><div style={S.turnCardTop}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{...S.statusDot,background:cfg.dot}}/><div><div style={S.turnClient}>{t.client}</div><div style={S.turnMeta}>{t.plant} · {truckList} · {t.m3}m³ · {timeRange}{t.concreteType&&` · ${t.concreteType}`}</div></div></div><div style={{...S.badge,background:cfg.bg,color:cfg.color}}>{cfg.label}</div></div>{!compact&&<div style={S.turnDetails}><span>📍 {t.destination}</span><span>👤 {t.operator}</span>{t.concreteType&&<span>🪨 {t.concreteType}</span>}{t.notes&&<span>📝 {t.notes}</span>}{t.destLat&&<span style={{color:"#10B981"}}>📌 Geocodificado</span>}</div>}{!compact&&<div style={S.turnActions}>{canAdv&&<button onClick={()=>onAdvance(t.id)} style={S.btnAdvance}>→ {STATUS_CONFIG[STATUS_FLOW[STATUS_FLOW.indexOf(t.status)+1]]?.label}</button>}{onEdit&&!["completado","cancelado"].includes(t.status)&&<button onClick={()=>onEdit(t)} style={S.btnSecondary}>Editar</button>}{onCancel&&!["completado","cancelado"].includes(t.status)&&<button onClick={()=>onCancel(t.id)} style={S.btnDanger}>Cancelar</button>}{onDelete&&["completado","cancelado"].includes(t.status)&&<button onClick={()=>onDelete(t.id)} style={S.btnDanger}>Eliminar</button>}</div>}{compact&&canAdv&&<div style={{marginTop:8}}><button onClick={()=>onAdvance(t.id)} style={S.btnAdvance}>→ {STATUS_CONFIG[STATUS_FLOW[STATUS_FLOW.indexOf(t.status)+1]]?.label}</button></div>}</div>);}
...
function TurnForm({initial,plants,trucks,users,concreteTypes,allTurns,currentUser,onSave,onClose}){const defaultEnd=initial?.endAt||(initial?.scheduledAt?initial.scheduledAt+3600000:Date.now()+3600000);const [form,setForm]=useState({client:initial?.client||"",plant:initial?.plant||plants[0]||"",trucks:initial?.trucks||(initial?.truck?[initial.truck]:[trucks[0]||""]),m3:initial?.m3||6,destination:initial?.destination||"",notes:initial?.notes||"",status:initial?.status||"pendiente",concreteType:initial?.concreteType||"",scheduledAt:initial?.scheduledAt||Date.now(),endAt:defaultEnd,destLat:initial?.destLat||null,destLng:initial?.destLng||null});const [conflicts,setConflicts]=useState([]);const set=(k,v)=>setForm(p=>({...p,[k]:v}));const neededTrucks=Math.ceil(form.m3/TRUCK_CAPACITY);function toggleTruck(tr){set("trucks",form.trucks.includes(tr)?form.trucks.filter(x=>x!==tr):[...form.trucks,tr]);}useEffect(()=>{const found=[];form.trucks.forEach(tr=>{const c=truckConflicts(allTurns,tr,form.scheduledAt,form.endAt,initial?.id||null);c.forEach(cf=>{if(!found.find(f=>f.id===cf.id))found.push({...cf,conflictTruck:tr});});});setConflicts(found);},[form.trucks,form.scheduledAt,form.endAt]);function toLocalDT(ts){return new Date(ts-new Date().getTimezoneOffset()*60000).toISOString().slice(0,16);}function fromLocalDT(s){return new Date(s).getTime();}const canSave=form.client&&form.destination&&form.trucks.length>0&&conflicts.length===0&&form.endAt>form.scheduledAt;return(<div style={S.modalOverlay}><div style={{...S.modal,maxWidth:600}}><div style={S.modalHeader}><h2 style={S.modalTitle}>{initial?"Editar Turno":"Nuevo Turno"}</h2><button onClick={onClose} style={S.modalClose}>✕</button></div><div style={{...S.formGrid,gap:14}}><div style={S.formGroup}><label style={S.label}>Cliente *</label><input value={form.client} onChange={e=>set("client",e.target.value)} style={S.input} placeholder="Nombre del cliente"/></div><div style={S.formGroup}><label style={S.label}>Destino *</label><input value={form.destination} onChange={e=>{set("destination",e.target.value);set("destLat",null);set("destLng",null);}} style={S.input} placeholder="Dirección de entrega"/></div><div style={S.formGroup}><label style={S.label}>Planta</label><select value={form.plant} onChange={e=>set("plant",e.target.value)} style={S.input}>{plants.map(p=><option key={p}>{p}</option>)}</select></div><div style={S.formGroup}><label style={S.label}>Tipo de Hormigón</label><select value={form.concreteType} onChange={e=>set("concreteType",e.target.value)} style={S.input}><option value="">— Sin especificar —</option>{concreteTypes.map(c=><option key={c} value={c}>{c}</option>)}</select></div><div style={S.formGroup}><label style={S.label}>Inicio</label><input type="datetime-local" value={toLocalDT(form.scheduledAt)} onChange={e=>set("scheduledAt",fromLocalDT(e.target.value))} style={S.input}/></div><div style={S.formGroup}><label style={S.label}>Finalización {form.endAt<=form.scheduledAt&&<span style={{color:"#EF4444"}}>⚠ debe ser posterior</span>}</label><input type="datetime-local" value={toLocalDT(form.endAt)} onChange={e=>set("endAt",fromLocalDT(e.target.value))} style={{...S.input,...(form.endAt<=form.scheduledAt?{borderColor:"#EF4444"}:{})}}/></div>{initial&&<div style={S.formGroup}><label style={S.label}>Estado</label><select value={form.status} onChange={e=>set("status",e.target.value)} style={S.input}>{Object.entries(STATUS_CONFIG).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select></div>}<div style={{...S.formGroup,gridColumn:"1/-1"}}><label style={S.label}>Volumen (m³) — {TRUCK_CAPACITY}m³/camión → necesita {neededTrucks} camión{neededTrucks!==1?"es":""}</label><input type="number" min="1" max="100" value={form.m3} onChange={e=>set("m3",Number(e.target.value))} style={{...S.input,maxWidth:120}}/></div><div style={{...S.formGroup,gridColumn:"1/-1"}}><label style={S.label}>Camiones ({form.trucks.length} sel. · cap: {form.trucks.length*TRUCK_CAPACITY}m³){form.trucks.length<neededTrucks&&<span style={{color:"#F59E0B",marginLeft:8}}>⚠ Faltan {neededTrucks-form.trucks.length}</span>}</label><div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:4}}>{trucks.map(tr=>{const sel=form.trucks.includes(tr);const hasConfl=conflicts.some(c=>c.conflictTruck===tr);return <button key={tr} onClick={()=>toggleTruck(tr)} style={{padding:"8px 14px",borderRadius:8,border:`2px solid ${hasConfl?"#EF4444":sel?"#4F8EF7":C.border}`,background:hasConfl?"#FEE2E2":sel?`${C.accent}22`:C.bg,color:hasConfl?"#EF4444":sel?C.accent:C.muted,fontWeight:sel?700:400,cursor:"pointer",fontSize:13,display:"flex",alignItems:"center",gap:6}}>🚛 {tr} {hasConfl&&"⚠"}</button>;})}</div></div>{conflicts.length>0&&<div style={{...S.formGroup,gridColumn:"1/-1"}}><div style={{background:"#FEE2E2",border:"1px solid #EF444455",borderRadius:8,padding:12}}><div style={{fontWeight:700,color:"#EF4444",marginBottom:6}}>⚠ Conflicto de horario</div>{conflicts.map(c=><div key={c.id} style={{fontSize:13,color:"#7f1d1d",marginBottom:4}}>{c.conflictTruck}: <strong>{c.client}</strong> ({fmtTime(c.scheduledAt)}{c.endAt?` → ${fmtTime(c.endAt)}`:""})</div>)}<div style={{fontSize:12,color:"#7f1d1d",marginTop:6}}>Cambiá el horario o seleccioná otros camiones.</div></div></div>}<div style={{...S.formGroup,gridColumn:"1/-1"}}><label style={S.label}>Notas</label><textarea value={form.notes} onChange={e=>set("notes",e.target.value)} style={{...S.input,minHeight:52,resize:"vertical"}} placeholder="Observaciones…"/></div></div><div style={S.modalFooter}><button onClick={onClose} style={S.btnSecondary}>Cancelar</button><button onClick={()=>{if(canSave)onSave(form);}} style={{...S.btnPrimary,opacity:canSave?1:.45,cursor:canSave?"pointer":"not-allowed"}}>{initial?"Guardar":"Crear turno"}</button></div></div></div>);}
